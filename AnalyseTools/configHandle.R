...
getProp  <-  function(configfile,type,cla,num=T){
    if(num)return( as.numeric(xmlAttrs( getNodeSet(xmlParse(configfile),paste("//",type,sep=""))[[1]])[cla] ))
    else return( xmlAttrs( getNodeSet(xmlParse(configfile),paste("//",type,sep=""))[[1]])[cla] )
}
